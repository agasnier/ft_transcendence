...
function MessagesList({messages, userId, role, myChannelRole, channelType, onEditMessage, onDeleteMessage}: MessagesListProps) {
    ...
    useClickOutside(editingId !== null, '[data-message-popover]', () => {
        setEditingId(null)
    })
                        ...
                        const content = msg.type === 'system' ? (
                            ...
                            <div key={msg.id} className={`group flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                                <div data-message-popover className={`flex flex-col min-w-0 p-3 rounded-2xl max-w-md shadow-sm relative
                                    ...
                                    ? 'bg-red-200 z-30'
                                    : isOwn ? 'bg-blue-200' : 'bg-white border-blue-100'}
            ...
            {confirmDeleteId !== null && (
                <div
                    ...
                    <div
                        ...
                        className="bg-white border rounded-xl shadow-lg p-4 flex flex-col gap-2 w-64">
                        <span className="text-sm text-gray-600">Supprimer ce message ?</span>
                        <div className="flex gap-2 justify-end">
                            <button
                                type="button"
