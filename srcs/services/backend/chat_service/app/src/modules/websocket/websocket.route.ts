...
import { wsAddChannelSocket, wsForceDisconnect, wsUserAvatarChanged, wsUserPseudoChanged } from './websocket.ws.js'
...
export async function websocketRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/internal/user-pseudo-updated/:userId', async (request, reply) => {
    const { userId } = request.params as { userId: string }
    const { pseudo } = request.body as { pseudo: string }
    wsUserPseudoChanged(Number(userId), pseudo)
    await reply.status(204).send()
  })
}
